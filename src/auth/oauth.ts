...
function startCallbackServer(expectedState: string): Promise<CallbackServer> {
  const { promise: serverPromise, resolve, reject } = Promise.withResolvers<CallbackServer>();
  let settled = false;
  ...
  const finish = (fn: () => void) => {
    ...
    server.on("error", (err: NodeJS.ErrnoException) => {
      if (err.code === "EADDRINUSE") {
        reject(
          new Error(
            "Port 51121 is already in use by another process. Please close the process using port 51121 and retry /login google-antigravity.",
          ),
        );
      } else {
        reject(err);
...
function parsePastedCallback(raw: string, expectedState: string): { code: string; state: string } {
  ...
  if (state !== expectedState) {
    throw new Error(
      "State mismatch: that callback is from a different sign-in. Re-run /login google-antigravity, sign in again, and paste the new URL.",
    );
  }
  return { code, state };
}
...
export async function loginAntigravity(
  ...
  try {
    ...
    if (!tokenData.refresh_token) {
      throw new Error(
        "No refresh token received. Re-run /login google-antigravity and allow offline access.",
      );
    }
